...
def train_target(args):
    ...
    args.name = args.type[args.s][0].lower()+'2' + args.type[args.t][0].lower()
    modelpath = args.output_src +'/'+ args.name + '/source_F.pt'
    netF.load_state_dict(torch.load(modelpath))
    modelpath = args.output_src +'/'+ args.name + '/source_C.pt'
    ...
    while iter_num < max_iter:
        ...
        with torch.no_grad():
            outputs_test_new = oldC(netF(inputs_test)).detach()
        netF.eval()
        oldC.eval()
        model.train()
        im_re_o.train()
